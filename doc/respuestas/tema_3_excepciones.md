...
### Respuesta
En Java, las excepciones controladas (checked) son aquellas que el compilador obliga a manejar o declarar con throws. Se utilizan cuando el error es previsible y recuperable, como ocurre al trabajar con archivos o conexiones. En cambio, las excepciones no controladas (unchecked) no obligan a declararse ni capturarse; suelen representar errores de programaciÛn. Estas ˙ltimas heredan de RuntimeException.

RuntimeException act˙a como clase base de las excepciones no controladas. Si una clase extiende de ella, el compilador no exige su tratamiento explÌcito. Por ejemplo, IOException (controlada) obliga a usar try-catch o throws, mientras que IllegalArgumentException (no controlada) puede lanzarse sin declararse. Incluso se podrÌan definir excepciones propias:
...
Situaciones donde se prefiere una excepciÛn controlada:

Lectura o escritura de archivos (ej. IOException).

ConexiÛn a una base de datos.

Acceso a red o recursos externos.

ValidaciÛn donde el usuario puede corregir el error.

Situaciones donde se prefiere una excepciÛn no controlada:

Paso de par·metros inv·lidos (IllegalArgumentException).

Acceso a una posiciÛn inexistente en un array (IndexOutOfBoundsException).

Uso de un objeto no inicializado (NullPointerException).

Errores lÛgicos que indican fallo de programaciÛn.

## 12. ¬øQu√© es y para qu√© se usa `throws`? ¬øPor qu√© es alternativa a capturar una excepci√≥n controlada?

### Respuesta
La palabra clave throws se utiliza en la declaraciÛn de un mÈtodo para indicar que puede lanzar una excepciÛn. En lugar de capturarla dentro del propio mÈtodo con try-catch, se declara que la responsabilidad de manejarla pasa al mÈtodo que lo invoque. Esto es obligatorio cuando se trata de una excepciÛn controlada y no se desea gestionarla localmente.

Se considera una alternativa a capturar la excepciÛn porque permite propagar el error hacia niveles superiores del programa. En lugar de resolver el problema en el mismo punto donde ocurre, se delega su tratamiento a otro mÈtodo que tenga m·s contexto o capacidad para decidir quÈ hacer. El compilador exige que quien llame al mÈtodo tambiÈn la capture o la vuelva a declarar.
...
En este caso, no se captura la IOException, sino que se declara con throws. El mÈtodo que llame a leerArchivo deber· manejarla o volver a propagarla.
...
### Respuesta
Se puede declarar un mÈtodo que abra un fichero y que no capture la excepciÛn, sino que la propague usando throws. En este caso, si el fichero no existe, se lanzar· una FileNotFoundException (que es controlada) y el mÈtodo que lo invoque deber· encargarse de manejarla. Sin embargo, puede utilizarse finally para asegurar el cierre del recurso si se llegÛ a abrir correctamente.

El bloque finally se ejecuta siempre, haya o no excepciÛn, lo que permite liberar recursos como en C cuando se usa fclose() tras comprobar errores. De esta forma, se combina la propagaciÛn de la excepciÛn con una gestiÛn correcta del recurso.
...
En este ejemplo no se captura la excepciÛn; simplemente se declara con throws y se deja que se propague. El finally garantiza que el fichero se cierre correctamente si llegÛ a abrirse.
...
### Respuesta
SÌ, es posible declarar en throws excepciones no controladas, como las que heredan de RuntimeException. El compilador lo permite, pero no es obligatorio hacerlo. A diferencia de las excepciones controladas, no existe la exigencia de declararlas ni de capturarlas.

El mÈtodo llamador no est· obligado a usar try-catch cuando se trata de una excepciÛn no controlada, aunque puede hacerlo si se desea manejar el error. Si no se captura, la excepciÛn se propagar· autom·ticamente hasta que sea gestionada o el programa termine. Esto es coherente con su finalidad: indicar errores de programaciÛn, como argumentos inv·lidos o accesos incorrectos.

Declararlas en throws puede tener sentido como documentaciÛn explÌcita, para dejar claro que el mÈtodo puede fallar en determinadas condiciones. Sin embargo, en la pr·ctica no suele hacerse, ya que las excepciones no controladas se entienden como fallos que deberÌan corregirse en el cÛdigo m·s que manejarse din·micamente.
...
### Respuesta
Se recomienda usar excepciones controladas (como IOException) cuando el error es previsible y recuperable, especialmente si depende de factores externos: archivos, red, bases de datos, etc. En estos casos, quien llama al mÈtodo puede tomar una decisiÛn razonable (reintentar, pedir otro fichero, mostrar un mensaje al usuario). El compilador obliga a tener en cuenta estas situaciones.

Se suelen usar excepciones no controladas (como IllegalArgumentException) cuando el error indica un fallo de programaciÛn, por ejemplo pasar un par·metro inv·lido o violar una precondiciÛn del mÈtodo. En este caso, no se espera que el programa ìse recupereî, sino que el desarrollador corrija el cÛdigo. Por eso no se obliga a capturarlas.

No todos los lenguajes tienen ambas opciones. Java es de los m·s conocidos que distingue entre controladas y no controladas. En muchos otros lenguajes (como C#, C++ o Python) todas las excepciones funcionan como no controladas, es decir, el compilador no obliga a declararlas ni capturarlas. La opciÛn m·s habitual en la mayorÌa de lenguajes modernos es, por tanto, el modelo de excepciones no controladas.
...
### Respuesta
SÌ tiene sentido lanzar excepciones dentro de un catch. Puede hacerse para transformar una excepciÛn en otra m·s adecuada al nivel de abstracciÛn del mÈtodo. Por ejemplo, si internamente ocurre una IOException, puede lanzarse una excepciÛn propia de la aplicaciÛn que oculte detalles tÈcnicos.

TambiÈn se puede relanzar la misma excepciÛn capturada usando throw e;. Esto permite, por ejemplo, realizar alguna acciÛn (como registrar el error o liberar recursos) y despuÈs dejar que la excepciÛn contin˙e propag·ndose. Es habitual cuando el mÈtodo no puede resolver el problema, pero necesita ejecutar cÛdigo adicional antes de que el error siga hacia arriba.

Ejemplo lanzando otra excepciÛn:
...
Ejemplo relanzando la misma excepciÛn:

try {
    leerArchivo();
} catch (IOException e) {
    System.out.println("Se produjo un error");
    throw e;  // Se vuelve a lanzar la misma excepciÛn
}
...
### Respuesta
Que una excepciÛn sea la ìcausaî de otra significa que una excepciÛn nueva se lanza, pero guardando internamente la excepciÛn original que provocÛ el problema. Esto permite no perder informaciÛn tÈcnica del error real, aunque se lance una excepciÛn m·s adecuada al nivel del programa. En Java esto se hace pasando la excepciÛn original como segundo argumento al constructor.

Este mecanismo se usa cuando se captura una excepciÛn de bajo nivel (por ejemplo, de acceso a fichero o base de datos) y se encapsula en una excepciÛn propia de la aplicaciÛn, m·s coherente con la lÛgica del sistema. AsÌ se mantiene la abstracciÛn: las capas superiores no necesitan conocer detalles internos, pero la informaciÛn sigue disponible.
...
Cuando una excepciÛn con causa se muestra por pantalla (por ejemplo con printStackTrace()), sÌ se ve la causa. Java muestra primero la excepciÛn principal y despuÈs indica ìCaused by:î seguido de la excepciÛn original y su traza, lo que facilita localizar el origen real del problema
